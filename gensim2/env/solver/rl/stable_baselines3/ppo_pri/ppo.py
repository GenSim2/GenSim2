...
from gensim2.env.solver.rl.stable_baselines3.common.preprocessing import get_action_dim
...
class Predictor(nn.Module):
    def __init__(self, o_dim, s_dim, encoder, tau=0.005):
        super(Predictor, self).__init__()
        self.predictor = nn.Sequential(
            *create_mlp(input_dim=o_dim, output_dim=s_dim, net_arch=[256, 256])
        )

        self.encoder = encoder
        self.tau = tau

    def get_spr_loss(self, obs):
        # Encode
        z = self.encoder(obs)
        s_label = obs["state"]
        s_pred = self.predictor(z)

        # Loss
        loss = ((s_label - s_pred) * (s_label - s_pred)).sum(dim=-1).mean()
...
class PPO(OnPolicyAlgorithm):
    ...
    def __init__(
        ...
        self.kl_scheduler = AdaptiveScheduler(
            kl_threshold=target_kl, min_lr=min_lr, max_lr=max_lr, init_lr=learning_rate
        )
    ...
    def _setup_model(self) -> None:
        ...
        self.target_features_extractor = features_extractor_class(
            self.observation_space, **self.policy_kwargs["features_extractor_kwargs"]
        ).to(self.device)
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                pred_loss = self.predictor.get_spr_loss(rollout_data.observations)

                rep_losses.append(pred_loss.item())
                ...
                update_learning_rate(self.predictor_optimizer, lr)
        ...
        self.logger.record("train/rep_loss", np.mean(rep_losses))
