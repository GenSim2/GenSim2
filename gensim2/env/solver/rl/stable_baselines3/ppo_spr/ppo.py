...
class SPR(nn.Module):
    def __init__(self, z_dim, a_dim, encoder, encoder_target, tau=0.005):
        ...
        self.dynamics = nn.Sequential(
            *create_mlp(input_dim=z_dim + a_dim, output_dim=z_dim, net_arch=[256, 256])
        )
        ...
        self.projection = nn.Sequential(
            *create_mlp(input_dim=z_dim, output_dim=z_dim, net_arch=[256])
        )
        self.projection_target = nn.Sequential(
            *create_mlp(input_dim=z_dim, output_dim=z_dim, net_arch=[256])
        )
        self.prediction = nn.Sequential(
            *create_mlp(input_dim=z_dim, output_dim=z_dim, net_arch=[])
        )
    ...
    def get_spr_loss(self, obs, actions, obs_target):
        ...
        # Normalize
        proj = F.normalize(proj, p=2.0, dim=-1, eps=1e-5)
        proj_target = F.normalize(proj_target, p=2.0, dim=-1, eps=1e-5)
...
class PPO(OnPolicyAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        self.SPR = SPR(
            ...
            self.target_features_extractor,
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                spr_loss = self.SPR.get_spr_loss(
