...
from gensim2.env.solver.rl.stable_baselines3.common.buffers import DictReplayBuffer
from gensim2.env.solver.rl.stable_baselines3.common.preprocessing import get_obs_shape
from gensim2.env.solver.rl.stable_baselines3.common.type_aliases import (
    DictReplayBufferSamples,
)
from gensim2.env.solver.rl.stable_baselines3.common.vec_env import VecEnv, VecNormalize
from gensim2.env.solver.rl.stable_baselines3.her.goal_selection_strategy import (
...
class HerReplayBuffer(DictReplayBuffer):
    ...
    def __init__(
        ...
        super().__init__(
            buffer_size, env.observation_space, env.action_space, device, env.num_envs
        )
    ...
    def sample(
        ...
        return self._sample_transitions(
            batch_size, maybe_vec_env=env, online_sampling=True
    ...
    def sample_goals(
        ...
        if self.goal_selection_strategy == GoalSelectionStrategy.FINAL:
            ...
            transitions_indices = np.random.randint(
                transitions_indices[her_indices],
        ...
        return self._buffer["next_achieved_goal"][
    ...
    def _sample_transitions(
        ...
        if online_sampling:
            ...
            if self.full:
                ...
                episode_indices = np.random.randint(
                    0, self.n_episodes_stored, batch_size
                )
        ...
        transitions["info"] = np.array(
            [
                ...
                for episode_idx, transition_idx in zip(
                    episode_indices, transitions_indices
                )
            ]
        )
    ...
    def _sample_her_transitions(self) -> None:
        ...
        observations, next_observations, actions, rewards = self._sample_offline(
    ...
    def truncate_last_trajectory(self) -> None:
        ...
        if current_idx > 0:
            ...
            self._buffer["done"][pos][current_idx - 1] = np.array(
