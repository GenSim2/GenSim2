...
from vec_env.maniskill2_utils_wrappers_obs import (
...
def register(
    ...
    if name in REGISTERED_ENVS:
        print(f"Env {name} already registered")
    ...
    def _register_env(cls):
        if uid in REGISTERED_ENVS:
            if override:
                ...
                print(f"Override registered env {uid}")
                REGISTERED_ENVS.pop(uid)
                registry.env_specs.pop(uid)
            else:
                print(f"Env {uid} is already registered. Skip registration.")
                return cls
