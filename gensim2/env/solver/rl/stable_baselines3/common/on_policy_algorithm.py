...
from gensim2.env.solver.rl.stable_baselines3.common.policies import ActorCriticPolicy
...
class OnPolicyAlgorithm(BaseAlgorithm):
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            # print('rollout step time: ', time.time() - start_time)
        ...
        if reward_gap > max(20.0, last_episode_reward * 0.2):
            # 0723 changed from True to False by baochen after talking with yuzhe, skip restore stage for bugs here
            self.need_restore = False
            self.current_restore_step = 0
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            # self.num_timesteps
            print("start collect_rollouts")
            continue_training = self.collect_rollouts(
                self.env, callback, self.rollout_buffer, n_rollout_steps=self.n_steps
            )
            print("finished collect_rollouts")
            ...
            if log_interval is not None and iteration % log_interval == 0:
                ...
                print("end dump and log interval")
