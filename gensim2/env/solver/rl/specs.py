...
class Array:
    ...
    def __repr__(self):
        return "Array(shape={}, dtype={}, name={})".format(
...
class BoundedArray(Array):
    ...
    def __init__(self, shape, dtype, minimum, maximum, name=None):
        ...
        if np.any(bcast_minimum > bcast_maximum):
            raise ValueError(
                _MINIMUM_MUST_BE_LESS_THAN_OR_EQUAL_TO_MAXIMUM.format(
    ...
    def __repr__(self):
        template = (
            "BoundedArray(shape={}, dtype={}, name={}, " "minimum={}, maximum={})"
        )
        return template.format(
            self.shape, repr(self.dtype), repr(self.name), self._minimum, self._maximum
        )
    ...
    def __eq__(self, other):
        if not isinstance(other, BoundedArray):
            return False
        return (
            super(BoundedArray, self).__eq__(other)
    ...
    def generate_value(self):
        return np.ones(shape=self.shape, dtype=self.dtype) * self.dtype.type(
            self.minimum
        )
...
_INVALID_STRING_TYPE = "Expected `string_type` to be one of: {}, got: {{!r}}.".format(
    _VALID_STRING_TYPES
)
