...
def merge_dict_spaces(dict_spaces: Sequence[gym.spaces.Dict]):
    reverse_spaces = merge_dicts([x.spaces for x in dict_spaces])
    for key in reverse_spaces:
        low, high = [], []
        for x in reverse_spaces[key]:
            assert isinstance(x, gym.spaces.Box), type(x)
            low.append(x.low)
            high.append(x.high)
        low = np.concatenate(low)
        high = np.concatenate(high)
        new_space = gym.spaces.Box(low=low, high=high, dtype=low.dtype)
        reverse_spaces[key] = new_space
    return gym.spaces.Dict(OrderedDict(reverse_spaces))
...
# Numpy
# -------------------------------------------------------------------------- #
def transformation_matrix_to_pose(transformation_matrix: np.ndarray) -> np.ndarray:
    ...
    Returns:
        Pose in the format of [x, y, z, qx, qy, qz, qw].
    """
    assert transformation_matrix.shape == (4, 4)
    pose = np.concatenate(
        [
            transformation_matrix[:3, 3],
            transforms3d.quaternions.mat2quat(transformation_matrix[:3, :3]),
        ]
    )
    return pose
