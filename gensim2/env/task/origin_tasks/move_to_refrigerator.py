...
class MoveToRefrigerator(GenSimBaseTask):
    ...
    def reset(self, random=False):
        ...
        if not random:
            set_openness(self.articulator, 1.0)
            set_default_pose(self.articulator, self.articulator_name, self.asset_id)
        else:
            set_random_pose(self.articulator, 1.0)
