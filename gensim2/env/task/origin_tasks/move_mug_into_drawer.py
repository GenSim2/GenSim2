...
class MoveMugintoDrawer(GenSimBaseTask):
    """Move a mug into a drawer."""

    def __init__(self, env):
        # For articulated tasks, articulator should be specified, while others should be None.
        super().__init__(env, articulator="drawer", rigid_body="mug", asset_id="19179")
    ...
    def get_progress_state(self):
        if check_openness(
            ...
            return 0.5
        # TODO: check if the object is planced into the drawer
    ...
    def get_reward(self):
        reaching_drawer_reward_coef = 1
        openness_drawer_reward_coef = 1

        reaching_ycb_reward_coef = 1
        reaching_ycb_goal_reward_coef = 1
        grasp_ycb_reward_coef = 1

        reaching_drawer_reward, reaching_drawer_done = l2_distance_reward(
            pos1=self.tcp.get_pos(), pos2=self.articulator.get_pos()
        )
        openness_drawer_reward, openness_drawer_done = joint_fraction_reward(
        ...
        if not openness_drawer_done:
            reaching_ycb_reward, grasp_ycb_reward, reaching_ycb_goal_reward = 0, 0, 0

        reward_list = [
            (
                reaching_drawer_reward_coef * reaching_drawer_reward,
                reaching_drawer_done,
            ),
            (
                openness_drawer_reward_coef * openness_drawer_reward,
                openness_drawer_done,
            ),
