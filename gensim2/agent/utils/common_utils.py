...
def mat2euler_batch(mat, axes="sxyz"):
    ...
    if repetition:
        ...
        ax = np.where(
            if_cy,
            np.arctan2(M[..., k, j], M[..., k, k]),
            np.arctan2(-M[..., j, k], M[..., j, j]),
        )
        ay = np.where(
            if_cy, np.arctan2(-M[..., k, i], cy), np.arctan2(-M[..., k, i], cy)
        )
...
def tabulate_print_state_and_write(
    state_dict, extra_dict, output_dir, train_epochs, drake=False
):
    dt_string = datetime.now().strftime("%d_%m_%Y_%H:%M:%S")
    file_name = "output_stat.txt" if not drake else "output_stat_drake.txt"
