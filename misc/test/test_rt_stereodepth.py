...
def build_scene(sim, renderer):
    ...
    material.diffuse_texture_filename = (
        "../third_party/SAPIEN/examples/assets/aligned/beer_can/texture.png"
    )
    material.metallic = 0.001
    material.roughness = 0.4
    builder.add_visual_from_file(
        "../third_party/SAPIEN/examples/assets/aligned/beer_can/visual_mesh.obj",
        material=material,
    )
    beer_can = builder.build()
    beer_can.set_pose(
        Pose(
            [0.370301, -0.246856, 0.0738802],
            [0.922673, -0.00379302, -0.00852731, 0.385469],
        )
    )
    ...
    material.base_color = [1.0, 1.0, 1.0, 1.0]
    material.diffuse_texture_filename = (
        "../third_party/SAPIEN/examples/assets/aligned/champagne/texture.png"
    )
    material.metallic = 0.01
    material.roughness = 0.2
    builder.add_visual_from_file(
        "../third_party/SAPIEN/examples/assets/aligned/champagne/visual_mesh.obj",
        material=material,
    )
    champagne = builder.build()
    champagne.set_pose(
        Pose(
            [0.182963, -0.277838, 0.0873777],
            [0.723872, 0.00616071, -0.00678847, -0.689874],
        )
    )
    ...
    material.base_color = [1.0, 1.0, 1.0, 1.0]
    material.diffuse_texture_filename = (
        "../third_party/SAPIEN/examples/assets/aligned/pepsi_bottle/texture.png"
    )
    material.metallic = 0.001
    material.roughness = 0.6
    builder.add_visual_from_file(
        "../third_party/SAPIEN/examples/assets/aligned/pepsi_bottle/visual_mesh.obj",
        material=material,
    )
    pepsi_bottle = builder.build()
    pepsi_bottle.set_pose(
        Pose(
            [0.392403, 0.0504232, 0.116739],
            [0.991259, -0.00145631, -0.00922613, 0.131601],
        )
    )
    ...
    material.roughness = 0.2
    builder.add_visual_from_file(
        "../third_party/SAPIEN/examples/assets/aligned/steel_ball/visual_mesh.obj",
        material=material,
    )
    steel_ball = builder.build()
    steel_ball.set_pose(
        Pose(
            [0.192034, 0.131187, 0.0170772],
            [0.949057, -0.0375225, 0.0676584, -0.305458],
        )
    )

    builder = scene.create_actor_builder()
    builder.add_visual_from_file(
        "../third_party/SAPIEN/examples/assets/aligned/water_bottle/water_bottle.glb"
    )
    water_bottle = builder.build()
    water_bottle.set_pose(
        Pose(
    ...
    material.base_color = [1.0, 1.0, 1.0, 1.0]
    material.diffuse_texture_filename = (
        "../third_party/SAPIEN/examples/assets/aligned/table/texture.png"
    )
    material.metallic = 0.1
    material.roughness = 0.3
    builder.add_visual_from_file(
        "../third_party/SAPIEN/examples/assets/aligned/table/visual_mesh.obj",
        material=material,
    )
    table = builder.build()
    table.set_pose(
...
def main():
    ...
    pcd = o3d.geometry.PointCloud(
        o3d.utility.Vector3dVector(pc[..., :3] * np.array([1, -1, -1]))
